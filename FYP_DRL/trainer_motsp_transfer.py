...
def train(actor, critic, w1, w2, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    now = now.replace(':', '_')
    bname = "_transfer_100cities_0start"
    ...
    for epoch in range(epoch_num):
        ...
        obj1s, obj2s = [], []
        x_circle = args.train_size/args.batch_size/200
...
# 新加 tensorboard
                writer = SummaryWriter("10start")
                writer.add_scalar("mean_reward_200bs", mean_reward, epoch*x_circle+(batch_idx+1)/200-1)
                writer.add_scalar("mean_loss_200bs", mean_loss, epoch*x_circle+(batch_idx+1)/200-1)
        ...
        print('Mean epoch loss/reward: %2.4f, %2.4f, %2.4f, obj1_valid: %2.3f, obj2_valid: %2.3f. took: %2.4fs '\
              ...
              np.mean(times)))
        writer.add_scalar("mean_reward_epoch", mean_reward, epoch-1)
        writer.add_scalar("mean_loss_epoch", mean_loss, epoch-1)
...
def train_tsp(args, w1=1, w2=0, checkpoint = None):
    ...
    print('w1=%2.2f,w2=%2.2f. : ' % (w1, w2), out)
    ...
    if args.task == 'tsp':
        ...
        for i in range(0,T+1):
            ...
            if i==0:
                ...
                checkpoint = 'tsp_transfer_100cities_0start/100_start/w_1.00_0.00'
                epoch_num = 50
                train_tsp(args, 1, 0, None)
            else:
                # Parameter transfer. train based on the parameters of the previous subproblem
                checkpoint = 'tsp_transfer_100cities_0start/%d/w_%2.2f_%2.2f'%(num_nodes, 1-w2_list[i-1], w2_list[i-1])
                epoch_num = 3
