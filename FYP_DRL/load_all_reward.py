...
# "../tsp_transfer_50run_500000_5epoch_20city/20"效果一般。应该再训练一遍
save_dir = "tsp_transfer_100run_500000_5epoch_40city/40"
# save_dir = "tsp_transfer_100cities_0start/100"
# param
update_fn = None
STATIC_SIZE = 4  # (x, y)
...
for i in range(0, N+1):
    ...
    with torch.no_grad():
        ...
        # t2
        # _all = t2_all + time.time() - t2
    _, obj1, obj2 = reward(static, tour_indices, 1-w[i], w[i])
    tours.append(tour_indices.cpu().numpy())
    objs[i,:] = [obj1, obj2]

# t1 = time.time()
# ac = os.path.join(save_dir, "w_%2.2f_%2.2f" % (1, 0),"actor.pt")
# cri = os.path.join(save_dir, "w_%2.2f_%2.2f" % (1, 0),"critic.pt")
# actor.load_state_dict(torch.load(ac, device))
# critic.load_state_dict(torch.load(cri, device))
# t1_all = t1_all + time.time()-t1
# calculate
...
# _, obj1, obj2 = reward(static, tour_indices, 1, 0)
# tours.append(tour_indices.cpu().numpy())
# objs[0,:] = [obj1, obj2]
...
scio.savemat("data_20_11to8/obj1_%d_%d.mat"%(STATIC_SIZE, D), {'obj1':obj1_matrix})
scio.savemat("data_20_11to8/obj2_%d_%d.mat"%(STATIC_SIZE, D), {'obj2':obj2_matrix})
scio.savemat("data_20_11to8/rl%d_%d.mat"%(STATIC_SIZE, D),{'rl':objs})
scio.savemat("data_20_11to8/tour%d_%d.mat"%(STATIC_SIZE, D),{'tour':np.array(tours)})
