...
% Copyright (c) 2016-2017 BIMK Group
    ...
    methods
        ...
        %              '-M',2,'-D',10)
            ...
            obj.Set();
            proStr = {'N','M','D','S','algorithm','problem','operator','evaluation','run','mode','outputFcn'};
            if nargin > 0
            ...
            obj1name = ['obj1_' int2str(obj.S) '_' int2str(obj.D) '.mat'];
            obj2name = ['obj2_' int2str(obj.S) '_' int2str(obj.D) '.mat'];
            obj3name = ['obj3_' int2str(obj.S) '_' int2str(obj.D) '.mat'];
            load(obj1name)
            load(obj2name)
            load(obj3name)
            obj4name = ['obj4_' int2str(obj.S) '_' int2str(obj.D) '.mat'];
            obj5name = ['obj5_' int2str(obj.S) '_' int2str(obj.D) '.mat'];
            load(obj4name)
            load(obj5name)
        ...
        %
        %   Note that it may lead to errors when the operator visit the
        ...
        function set.S(obj,value)
            obj.S = obj.Set('S',value,'int','number of static ''-S''',3);
        end
...
%                          func2str(obj.algorithm),func2str(obj.problem),obj.M,obj.run,obj.evaluated/obj.evaluation*100,obj.runtime);
            if obj.evaluated >= obj.evaluation
                folder = fullfile('Data',[func2str(obj.algorithm) '_' int2str(obj.S)]);
                [~,~]  = mkdir(folder);
                Population     = obj.result{end};
                PF             = obj.PF;
                metric.runtime = obj.runtime;
                save(fullfile(folder,sprintf('%s_%s_M%d_%d_%d.mat',func2str(obj.algorithm),func2str(obj.problem),obj.M,obj.D,obj.evaluation)),'Population','PF','metric');
            end
        end
        ...
        function value = Metric(metric,Population,PF)
            ...
            if ~isempty(Population) && ~isempty(PF)
                value = metric(Population.objs,PF);
            else
                value = NaN;
            end
        end
